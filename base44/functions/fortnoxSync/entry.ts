...
// Get OAuth token from Fortnox
async function getFortnoxToken(clientId, clientSecret) {
...
Deno.serve(async (req) => {
  try {
    ...
    const { syncType, clientId, clientSecret } = await req.json();

    if (!syncType || !clientId || !clientSecret) {
      return Response.json({ error: 'Missing required parameters' }, { status: 400 });
    }

    // Get Fortnox token
    const accessToken = await getFortnoxToken(clientId, clientSecret);
