...
export default function ScanPage() {
  ...
  const handleModeSelect = (selectedMode) => {
    ...
    if (selectedMode === "barcode") {
      ...
      setStep("capture");
    } else if (selectedMode === "pending_verification") {
      setStep("capture"); // Capture image first, then fill minimal form
    } else {
      setStep("capture");
    }
  };
  ...
  const handleImageCaptured = async (files) => {
    ...
    try {
      ...
      if (mode === "pending_verification") {
        ...
        // Run Kimi in background — updates form when done
        base44.functions.invoke('parseImage', { fileUrls: urls })
          .then(analysisResult => {
            ...
            if (batchNumber) {
              setExtractedData(prev => ({ ...prev, batch_number: batchNumber }));
            }
          })
          .catch(e => console.log("Kimi background analysis failed:", e));
        return;
      }
  ...
  "analysisGroups": [
    {
      ...
      try {
        ...
        // Check specifically for pending_verification articles matching the batch
        if (data.batch_number) {
          const pendingMatches = uniqueMatches.filter(m => m.article.status === 'pending_verification');
          if (pendingMatches.length > 0) {
            const pm = pendingMatches[0];
            const batchMatch = calculateBatchMatch(data.batch_number, pm.article.batch_number || '');
            if (batchMatch >= 70) {
              setPotentialMatches([{ ...pm, batchMatchPercentage: batchMatch }]);
              setExistingArticle(pm.article);
              setExtractedData({ ...data, image_urls: urls });
              setConfidences(confs);
              setProgress(100);
              setShowMatchConfirm(true);
              toast.info(`Hittade väntande artikel: ${pm.article.batch_number}`);
              return;
            }
          }
        }
      ...
      // For pending_verification mode, go to the pending form
      if (mode === "pending_verification") {
        setStep("pending_form");
        ...
        <AnimatePresence mode="wait">
          ...
          {step === "capture" && (
            ...
            >
              ...
              {mode !== "repair" && mode !== "pending_verification" && (
                <div className="pt-2">
                  <Button
                    variant="outline"
                    onClick={() => {
          ...
          {step === "repair_match" && (
            <RepairMatchStep
              ...
              setRepairArticle={setRepairArticle}
          ...
          {step === "repair_label" && (
            <RepairLabelStep
              ...
              onPrint={handlePrintRepairLabel}
              onReset={handleReset}
            />
          )}
            ...
            >
              ...
              {imageUrls.length > 0 && (
                <div className="mb-4">
                  <div className="grid grid-cols-3 gap-2">
                    {imageUrls.map((url, index) => (
                      <img key={index} src={url} alt={`Bild ${index + 1}`} className="w-full h-24 object-cover rounded-lg bg-slate-900" />
                    ))}
                  </div>
                </div>
              )}
              <PendingVerificationForm
                imageUrls={imageUrls}
                extractedBatch={extractedData.batch_number || ""}
